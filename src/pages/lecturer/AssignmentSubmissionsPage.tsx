...
const AssignmentSubmissionsPage: React.FC = () => {
  ...
  const handleDownload = async (submissionId: string, studentName: string) => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `Submission_${studentName.replace(/\s+/g, '_')}.pdf`);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <Link to={`/lecturer/modules/${assignment.module.id}/assignments`} className="flex items-center text-gray-500 dark:text-gray-400 hover:text-primary-600">
            <ArrowLeft className="h-4 w-4 mr-1" /> Back to Assignments
          </Link>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Submissions: {assignment.title}</h1>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="font-bold text-gray-900 dark:text-white">{new Date(assignment.dueDate).toLocaleString()}</p>
          </div>
        </div>
                ...
                {submissions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                    <td className="px-6 py-4">
                      <p className="font-medium text-gray-900 dark:text-white">{sub.student.user.firstName} {sub.student.user.lastName}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{sub.student.user.registrationNumber}</p>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      {sub.marks !== null ? (
                        <span className="font-bold text-primary-600 dark:text-primary-400">{sub.marks}</span>
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <button
                          ...
                          className="text-sm font-bold text-primary-600 dark:text-primary-400 hover:text-primary-700"
                        >
      ...
      {markingId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-md w-full p-6">
            <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">Mark Submission</h3>
            <form onSubmit={handleMark} className="space-y-4">
              <div>
                ...
                <input
                  type="number"
                  required
                  className="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-lg p-2 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
              ...
              <div className="flex justify-end space-x-3 pt-4">
                <button
                  ...
                  className="px-4 py-2 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg transition"
                >
                  Cancel
                </button>
                <button
